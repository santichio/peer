...
- `gitflow-workflow` — drives a change through gitflow with `git` + `gh`; stops at PR-ready.
...
- Skills now follow the SKILL.md convention (flat `skills/<name>/SKILL.md` folders,
  `name`/`description` frontmatter) instead of the base-document template. `gitflow-workflow`
  was restructured into this format.
